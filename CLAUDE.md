...
**OG images** — every blog post gets a 1200×630 PNG prerendered at build time by Satori (+ `@resvg/resvg-js` for PNG output). The template lives in `src/lib/og.ts` as plain object trees (no JSX/TSX configured); the dynamic route `src/pages/og/[...slug].png.ts` uses `getStaticPaths` over the blog collection and emits `dist/og/<slug>.png`. JetBrains Mono Regular + Bold are vendored under `src/assets/fonts/` and loaded from `process.cwd()` — the endpoint only ever runs at build time, so the working directory is the project root. Title font auto-shrinks across four buckets by character count (96/84/68/56px). The card uses absolute-positioned scanline + vignette layers (Satori ignores CSS pseudo-elements), and the logo sits absolutely at the start of the title block with `text-indent` on the text so wrapped lines reclaim the full width. `Base.astro` accepts an optional `image` prop that `Post.astro` sets to the per-post URL; `BaseHead.astro` falls back to `/og-image.png` for non-post pages.
